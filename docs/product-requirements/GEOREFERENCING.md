...
| Tier 2, a modern geocoder: Google; Mapbox is not used | tier 2 | Google receives the modernized name, or the literal as today when tier 1 finds nothing to modernize, where G34 applies, in requests PLAN 4.8 governs; only its place ID, outcome and fingerprint are kept (G26); its coordinates are never stored, traced or logged, and tier 3, derivations and the georeference never use them (the coordinator's readings of G35 and PLAN 4.8) |
