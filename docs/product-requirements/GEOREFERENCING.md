...
| D14 | What a geography lookup confirms under G20 when readers disagree on a toponym's spelling | a) the place and the spelling; b) the place only, the spelling stays with the first pass | b: otherwise a reader's silent correction ("Chimaltenango") replaces the label's spelling ("Chimaltenago") | `field_outcomes` when readers disagree (3.3) |
