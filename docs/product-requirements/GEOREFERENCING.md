...
| Storage | only Google's place ID, with the outcome and the response fingerprint (G26) | CC0 and CC BY coordinates with attribution; the verbatim text is immutable (PRD 12.4) |
...
| Cost | `execute_one` refuses a non-zero cost reservation (`evidence_harness.py` 424-425) | open sources are free; Google is tier 2 (G35), so each of its calls reserves its cost (G30; the coordinator's ruling on the mechanism, PLAN 4.3) |
...
| Tier 3, spatial filtering: the final point-radius uncertainty | tier 3 (3.8) | in-house (D13, the coordinator's ruling); while D4 and D5 are held (the coordinator's rulings, PLAN 2.3), the museum-points check is off and sends nothing, and the other checks record findings that change no outcome |
...
| GADM through GBIF's reverse geocoder; geoBoundaries; CONRED's COD-AB file on HDX | containment for derived county and city (G37); polygons for radials | anonymous; limits undocumented [S14] | GADM non-commercial, no redistribution without permission; geoBoundaries licensed per file, its Philippine units CC BY 3.0 IGO and its Guatemalan departments ODbL (from OpenStreetMap); COD-AB CC BY 3.0 IGO [S18] | research only, from GBIF's GADM-based reverse geocoder: the Apo summit area sits on a three-province boundary (North Cotabato at distance 0, Davao del Sur about 90 m away) | GADM is not used, not even as a measurement (the coordinator's ruling, PLAN 4.8); containment uses geoBoundaries' simplified Philippine files, with the circle widened by their stated simplification error, and CONRED's COD-AB file for Guatemala, never geoBoundaries' OpenStreetMap file, each read from the project's storage and credited per file (the coordinator's rulings, PLAN 4.8) |
...
| Temporal validity | valid 1.0; undated 0.6. While D5 is held, a unit that had ended by the date is not valid and is never scored; a label-lag value waits on D5's tolerance | 0.15 |
...
| Geology | containment, stratigraphic plausibility (Macrostrat, PBDB), `GeologicalContext` terms | Macrostrat and PBDB send nothing until a ruling adds them to PLAN 4.8; Mindat only after D10 and a ruling that adds it to PLAN 4.8 |
...
| georeferenceSources | GeoNames GT dump (CC BY 4.0); Wikidata Q1523937 (CC0); Copernicus GLO-30 for the elevation check (D11); CONRED's COD-AB file for containment (CC BY 3.0 IGO); each with its access date |
...
| geoBoundaries' simplified Philippine files | CC BY 3.0 IGO, per file (NAMRIA, PSA and OCHA Philippines, via geoBoundaries) [S18] | yes, credited | derived county and city (G37), the circle widened by the stated simplification error |
| CONRED's COD-AB file for Guatemala, via HDX | CC BY 3.0 IGO [S18] | yes, credited | derived county and city (G37) |
...
QID = re.compile(r"Q[1-9][0-9]*")  # S4's filter's pattern for 4.8's Q-number, matched whole
...
class Candidate(BaseModel):  # becomes a PlaceCandidate, or feeds a GeoreferenceCandidate (HAR-004, HAR-014)
    ...
    validity: Literal["valid", "undated", "not valid"] = "undated"
    label_lag_days: int | None = None  # a finding: the name outlived its unit by this gap (3.2)
...
def valid_on(valid_from: str | None, valid_to: str | None, day: date) -> tuple[str, int | None]:  # 3.2
    ...
    if valid_from and date.fromisoformat(valid_from[:10]) > day:
        return "not valid", None  # founded after the collection date
    if valid_to and (ended := date.fromisoformat(valid_to[:10])) < day:
        return "not valid", (day - ended).days  # no tolerance widens the dates while D5 is held
    return "valid", None
...
class WikidataTier1:  # discovery through the Action API, then one SPARQL enrichment by QID
    ...
    async def run(self, state: "State", gate: HttpGate) -> SourceResult:
        ...
        for row in json.loads(body).get("results", {}).get("bindings", []) if ids else []:
            ...
            validity, lag = valid_on(value.get("validFrom"), value.get("validTo"), state.day)
            candidates.append(Candidate(
                source_id=f"wikidata:{qid}", feature_id=qid, role="historical", storable=True,
                point=None if lat is None else (lat, lon), validity=validity, label_lag_days=lag))
...
Cost: the open sources cost USD 0, inside G9's USD 25. Google Geocoding is free for the first 10,000 Essentials calls a month and USD 5.00 per 1,000 after that [S11]; as tier 2 (G35), each call reserves its cost (G30; the coordinator's ruling on the mechanism, PLAN 4.3).
...
| D3b | May an expedition itinerary a curator has confirmed count as evidence that settles a place, when the collector, the place and the date all match it? | a) Yes. b) No: it is shown to reviewers only. | a. **Decided by the owner (G36):** a confirmed itinerary match settles a field. On the Apo slide it refines the place, which may otherwise settle from gazetteer evidence or stay unresolved, and which Mount Apo is meant is itself unsettled (the coordinator's reading of G36, its correction of 00:43Z on 2026-09-25, PLAN 2.3); the refinement waits while the itinerary is unconfirmed | the refinement of the Apo slide's georeference (105526327); the itinerary check |
...
### Held and waiting (the coordinator's rulings)

D4 and D5 were put to the owner and dismissed, so they are held: until decided, D4's occurrence check is off and sends nothing, and D5's checks record their measurements as findings and never change an outcome (coordinator, PLAN 2.3). D8, D10 and D12 wait for their phase (coordinator, 2026-09-24), except Getty TGN, wanted now (the coordinator's reading of G35). D11 and D13 are the coordinator's engineering rulings.
...
- [S18] GADM license, https://gadm.org/license.html; geoBoundaries, https://github.com/wmgeolab/geoBoundaries, with each file's license and sources in its metadata, such as https://www.geoboundaries.org/api/current/gbOpen/PHL/ADM3/; CONRED's COD-AB for Guatemala, https://data.humdata.org/dataset/cod-ab-gtm. The per-file licenses were checked 2026-09-24.
