...
**Status:** The owner asked for the retrospective tool to be fully implemented (G34, 2026-09-24) and set its design the same day (G35 to G40; the owner's diagram is in 1.2), and S8 builds it. D4 and D5 are held, D8, D10 and D12 wait for their phase except for Getty TGN (G35), and D11 and D13 are the coordinator's engineering rulings. The Google Maps geography tool (G10) stays in production until the acceptance lab shows that this tool resolves the pilot slides at least as well (coordinator, 2026-09-24).
...
|---|---|---|
| E. slope Mt. McKinley, 6,400 ft and 3,300 ft, Sept. 1946 (105526321-326) | `ambiguous`: needs human review until a curator confirms an entry (G36) | no gazetteer match; an unconfirmed crosswalk hypothesis (Mount Talomo); the museum's points lie far from the labels' elevations |
...
|---|---|---|
| Tier 1, historical gazetteers (GeoNames, Wikidata, Getty TGN): the historical name and when it was in use, returning modern equivalents and historical extents | the curated entries (tier 0) and tier 1 (Wikidata, the GeoNames dumps, NGA GNS), with the anachronism filter (3.2) and the hierarchy gate | a curated entry settles a field only once a curator confirms it (G36); Getty TGN joins now (G35) |
| Tier 2, a modern geocoder: Google; Mapbox is not used | tier 2 | Google receives the modernized name with the surrounding context, or the literal as today when tier 1 finds nothing to modernize, where G34 applies; only its place ID, outcome and fingerprint are kept (G26); its coordinates are never stored, traced or logged, and tier 3, derivations and the georeference never use them (the coordinator's readings of G35 and PLAN 4.8) |
| Tier 3, spatial filtering: the final point-radius uncertainty | tier 3 (3.8) | in-house (D13, the coordinator's ruling); the checks record measurements and change no outcome while D4 and D5 are held |

The owner also set how values are layered and filled in:
- **Layers (G38).** Every value belongs to a layer: image, transcription, verbatim, the harness's settled answer, a value derived from other fields. A later layer never erases an earlier one.
- **Derived fields (G37, revising G22).** A field the label leaves out is derived from settled fields with its authority and evidence, and it counts for mandatory fields. County and city are derived when the whole uncertainty circle lies inside one unit, and a missing elevation from Copernicus GLO-30 at the settled location. While the fields it depends on are unsettled, the field waits for review.
- **The harness's job (G36, G40).** The harness reads everything transcribed and keeps looking things up until it settles or derives a field, or shows it can't. It draws no conclusion without evidence.
...
- Every request to a gazetteer or to Google carries place text only: the name and the same reading's place fields, never collectors, dates or other transcribed text.
...
| Google Geocoding API | T2: the modernized name with its context (G35) | key; 10,000 free Essentials calls a month, then USD 5.00 per 1,000; only `country` and `postal_code` components restrict [S11] | coordinates cached 30 days at most, `place_id` indefinitely, no content derived from Maps Content [S10]; G26 keeps only place ID, outcome and fingerprint | not called: no key yet, and paid calls are refused | tier 2 (G35); place ID, outcome and fingerprint only (G26) |
| Mapbox Geocoding v6 | none | token; 1,000 requests a minute [S19] | no redistribution, even of paid permanent geocodes [S19] | not called | not used (G35) |
...
stateDiagram-v2
    ...
    state Resolve {
        ...
        Tier2 --> Tier3: modernized name and context to Google
...
| Tier 1, curated | the crosswalk and itineraries S8 drafts with sources, feeding tier 1 (G35); the museum's published points are measurements while D4 is held | only curator-confirmed entries settle a field (G36) | an unconfirmed entry is a hypothesis: `ambiguous` at best |
...
| Measurements | checks typed `supports`, `conflicts` or `not_assessable`: containment, DEM elevation (a stated elevation stays as written; feet become meters only inside the comparison), itinerary, occurrences, profile checks (3.7) | none while D4 and D5 are held: each check records its measurement as a finding | never changes an outcome |
| Derive | fields the label leaves out, from settled fields: county and city when the whole uncertainty circle lies inside one unit, a missing elevation from Copernicus GLO-30 at the settled location (G37, D11); recorded as derived, with authority and evidence (G38) | the fields it derives from are settled | the field waits for needs human review (G37) |
...
- "Jones Farm": a private name missing from gazetteers is `no_match` for the feature. The owner decided the rest (G38): the county settles, the named spot stays verbatim, and the location is derived from the county at county precision, its radius covering the county, and recorded as derived. S8 had proposed that such a fallback go to review.
...
| several unrelated features; only an unconfirmed curated entry (G36); a coordinate only from a non-storable source | `ambiguous`, with reasons | needs human review (conflicting evidence; missing or ambiguous field) |
| a named place not found, with its enclosing unit settled ("Jones Farm, Cook Co.") | the unit's field succeeds; the location is derived from the unit at its precision and recorded as derived (G38) | cleared if every other gate passes; the named spot stays verbatim |
...
| georeferenceSources | GeoNames GT dump (CC BY 4.0); Wikidata Q1523937 (CC0); Copernicus GLO-30 for elevation (D11); geoBoundaries' open release for containment (CC BY 4.0); each with its access date |
...
| GeoNames dumps; NGA GNS; Getty TGN | CC BY 4.0 [S3]; citation requested [S4]; ODC-By 1.0 [S5] | yes, attributed | yes, credited (G35); TGN now (G35) |
| DEMs; Macrostrat | public domain or free with attribution [S21]; CC BY 4.0 [S23] | yes | as evidence only |
| GBIF occurrences (FMNH) | CC0, CC BY or CC BY-NC per dataset [S9] | as evidence | measurements only while D4 is held |
...
class Candidate(BaseModel):  # becomes a PlaceCandidate, or feeds a GeoreferenceCandidate (HAR-004, HAR-014)
    ...
    museum_published: bool = False  # FMNH's own published point: a measurement while D4 is held
...
|---|---|---|---|
| 1 | Owner decisions: D1 to D3, D6, D7 and D9 (G35 to G39) and D14 and D15 (G27, G34) are decided; D4 and D5 are held; D11 and D13 are the coordinator's rulings; D8, D10 and D12 wait, except Getty TGN | owner | recorded here, 2026-09-24 |
| 2 | Confirm or reject the McKinley crosswalk entry and the 1946-47 expedition itinerary from S8's review sheets, routed through the owner | the Insects collection manager or a curator they name (G36) | a confirmed entry lands in a pull request that cites the owner's recorded confirmation, with a test that an unconfirmed entry never settles; the repository records the confirming role and date, never a name (G36, PLAN 4.8) |
...
| 7 | Replace the G10 Google tool behind the same `geography_lookup` interface once the owner has ruled on D1 and the acceptance lab shows this tool resolves the pilot slides at least as well; until then the Google tool stays in production (G6) | S8, S3, S7, owner | the pilot's geography fields call this tool, and Google only as D1 allows (coordinator, 2026-09-24) |
...
Risks: Wikidata throttling (HTTP 429 with `Retry-After` after S8's second request from a shared address [S16]) is met by discovery through the Action API, a local cache and retries through the workflow; Getty's move to token-gated endpoints [S5] by the access the owner arranges (G35); license obligations for GeoNames, NGA, Getty and the DEMs by attribution in `georeferenceSources` and dataset metadata (D2); sparse historical names by a tier-0 crosswalk curated by people; and the museum's repetition of one retrospective point across hundreds of records by recording those points as measurements while D4 is held.

Other workstreams: S8 builds the tool in its own modules behind S4's T3a interface (G34), with Google as tier 2 through S4's Google tool (G35), and the geographic derivations (containment, Copernicus elevation, gazetteer naming) as derivation results that S4 consumes (coordinator, 2026-09-24); S4 keeps the interface and the Google tool. S5 stores one ToolCall row per sub-call (G23); record fields for the georeference come after the pilot (G39). S3 maps the geography fields to `geography_lookup` in `field_tools`, selects checks per collection and implements profile inheritance (`COLLECTION_HIERARCHY.md` 73-76).

## Owner decisions

The owner decided D1 to D3, D6, D7 and D9 on 2026-09-24 (G35 to G39, quoted below with the coordinator's readings) and dismissed D4 and D5, which are held: until decided, those checks record their measurements as findings and never change an outcome. D11 and D13 are engineering choices the coordinator ruled on the same day. D8, D10 and D12 wait for their phase, except Getty TGN (G35), and nothing is built for them. The options and S8's recommendations below are as the owner was asked. D14 and D15 are decided: D14 on 2026-09-23 (G27), ahead of the rest because S4 was building G20, and D15 on 2026-09-24 (G34), when the owner also asked for this tool to be fully implemented.

| # | Question | Options | S8 recommendation | What it blocks |
|---|---|---|---|---|
| D1 | What role does Google play inside this tool? (G10, G26, G34; PRD 12.4) | a) Google keeps its role under G34: a Google-only match clears the field with the place ID alone, including a match one letter off that fits the other place fields, and open sources add names, historical and modern units and the point-radius. b) Google confirms only that a place exists: open sources settle the place fields, and a Google-only match no longer clears a field on its own when the label predates a jurisdiction change, such as a 1946 "Davao Prov.". c) Drop Google and use open sources only. d) Negotiate enterprise terms so that Google's names and coordinates may be kept. | b. **It conflicts with G34**, which the owner decided on 2026-09-24: under G34 a Google match one letter off that fits the other place fields clears the field with its place ID, and under b such a match would no longer clear a field on its own when the label predates a jurisdiction change, because Google knows only today's units and G26 leaves a reviewer no name to check. a keeps G34 as it stands, b narrows it for such labels, c retires it with Google, and d would reopen G26. **Decided by the owner (G35, 2026-09-24)** with a design of their own, the three tiers in 1.2: Google is tier 2, receiving the modernized name with its context, and Mapbox is not used | the tier-2 Google step and how a Google-only match counts; the paid-call path; the PRD 12.4 source table |
...
| D12 | Should the project get the sources that need a token or an account? | a) Not yet: defer Getty's new gateway, a World Historical Gazetteer token and a GeoNames account (the dumps need none). b) Request them now through the owner actions. | a. Waits for its phase (coordinator, 2026-09-24); nothing is built for it; Getty TGN is wanted now (G35) | nothing in Phase 1 |
...
- **Held and waiting.** D4 and D5 were put to the owner and dismissed, so they are held: until decided, those checks record their measurements as findings and never change an outcome. D8, D10 and D12 wait for their phase, except Getty TGN (G35). D11 and D13 are the coordinator's engineering rulings.
