...
| Tier 2, a modern geocoder: Google; Mapbox is not used | tier 2 | Google receives the modernized name with the surrounding context, or the literal as today when tier 1 finds nothing to modernize, where G34 applies; only its place ID, outcome and fingerprint are kept (G26), and its coordinates serve inside a run only (the coordinator's reading of G35) |
...
S8 builds the geographic derivations (containment, Copernicus elevation, gazetteer naming) as derivation results behind S4's interface, which S4 consumes (coordinator, 2026-09-24).
...
| Tier 2 | Google receives the modernized name with its context, or the literal when tier 1 finds nothing to modernize (G35, G34); modern units through successors and containment; the coordinate of record from openly licensed sources, credited | a storable source for a stored coordinate (G35) | Google's coordinates serve inside the run only; the georeference is `ambiguous`, reason `coordinate_not_storable` |
...
| 2 | Confirm or reject the McKinley crosswalk entry and the 1946-47 expedition itinerary from S8's review sheets, routed through the owner | the Insects collection manager or a curator they name (G36) | confirmed entries recorded with the confirming role and date, never a name (G36) |
