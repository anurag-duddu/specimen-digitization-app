...
Hash follow-up: history page sha256 is the original retained snapshot hash, not the hash of a reconstructed model with newer default fields. Review references likewise bind the retained raw run's canonical JSON, and reference reads verify that stored run hash. A regression fixture with legacy missing defaults confirms that hydration does not change the advertised stored hash. The enabled history suite passes all five tests including actual SQL/TCP 110-review/replay paging; SQL test requests now use unique intake idempotency keys so repeated test runs on one disposable cluster remain isolated.
