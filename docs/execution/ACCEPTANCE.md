...
| P0-08 | Pass local: `test_application.py::test_lookup_http_failure_taxonomy`, `test_lookup_timeout_and_malformed`, `test_authority_runtime.py`, `test_parties.py` (`test_geography.py` went with the GADM adapter, #216) | B+D+Q combined actual HTTP/SQL replay; live approved authority access/response validation external. |
