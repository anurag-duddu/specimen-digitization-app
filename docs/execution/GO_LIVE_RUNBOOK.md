...
2. Run the human-review checker over the unchanged 45-case ledger plus the
   ...
   > G1, G2, G9 and G11. The ten pilot specimens are processed one at a time, in
   > order (PLAN section 8), and a record the harness resolves is cleared
   > without a human, so the administrator reviews only what the queue sends to
   > human review. The rest of items 1 and 2 stands, including every UI and live
