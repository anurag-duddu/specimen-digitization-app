...
r=Path('/tmp/specimen-qa-ee4bec8-heic');m=json.loads((r/'manifest.json').read_text());source=Path(m['source']).read_bytes();c=httpx.Client(base_url='http://127.0.0.1:8124',headers={'Authorization':'Bearer test-only-local-token'});prefix='/v1/organizations/00000000-0000-4000-8000-000000000001';p=prefix+'/specimens/'+m['specimen_id']
for _ in range(100):
 w=c.get(p+'/workspace').json()
 if w['status'] in ('completed','processing_blocked'):break
 time.sleep(.05)
assert w['revision']>20 and len(w['run']['regions'])==1
...
assert not w['run']['human_approved'];(r/'verified-workspace.json').write_text(json.dumps(w));(r/'verified-crop.png').write_bytes(crop)
result={'saved_revision':w['revision'],'regions':1,'decoded_source_basis':[80,120],'saved_original_basis_crop':[5,7,40,50],'clockwise_quarter_turns':1,'original_bytes_exact':True,'crop_numpy_oracle_exact':True,'initial_history_asset_exact':True,'human_approved':False,'status':w['status'],'disposition':w['disposition']};(r/'independent-results.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
