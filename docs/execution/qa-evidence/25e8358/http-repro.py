...
def check(name,actual,expected):
 results.append({'case':name,'actual':actual,'expected':expected,'status':'passed' if actual==expected else 'failed'}); (p/'repair-http-evidence.json').write_text(json.dumps(results,indent=2))
...
(p/'repair-http-evidence.json').write_text(json.dumps({'candidate':'25e8358','mode':'synthetic','transport':'SQL Connect9519/PostgreSQL5569 with local immutable blobs','specimen_id':ident,'results':results},indent=2))
(p/'workspace-positive.json').write_text(json.dumps(w,indent=2))
print(json.dumps({'results':[{k:v for k,v in r.items() if k in ('case','status','http')} for r in results],'evidence':str(p/'repair-http-evidence.json')},indent=2))
