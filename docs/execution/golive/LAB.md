...
| `--timeout-seconds` | 1800 | stop driving the record after this; a finite positive number |
...
1. **Preflight.** The subject id is `subject_` and digits. The values file is
   located and read first (see Secrets). Load and allowance are checked. The inference switch and token are present; only presence is
   read, never a value.
...
7. **Report.** The files below, written even when a phase or the lane's
   teardown fails. A Ctrl-C is recorded as a failed phase, and the run is
   ...
   replaces it. The test client swallows a Ctrl-C raised during one of the
   app's own requests (a 500 if the app had not started its response,
   otherwise the endpoint's own response), so the lane holds it until the
   request returns and then raises it. After a Ctrl-C during `/complete`'s
   drain, the drain's later paid calls still run before the request returns,
   and the run stays held at `--max-run-usd`. The subject report is rebuilt from every run of that
...
| 7 Agentic harness | the run records tool calls with typed outcomes | failed on a GBIF occurrence request while D4 is held. PLAN 4.8's table lists GBIF only for species match (G23) and the held occurrence search. A request is: a GBIF record (by `source`, `provider`, `source_id`, `tool` or `tool_id`) whose identity or path names the occurrence search, with or without a host, or which carries occurrence query keys; any record naming `api.gbif.org/v1/occurrence`; a plain or escaped `"museum_published": true`, or an occurrence signal that supports or conflicts; a receipt blob showing any of these, as text or as a record; or a request the runner counts, before sending, at `bounded_http` in the parent or on an injected `httpx` client, to `api.gbif.org` with the `/v1/occurrence` path or occurrence query keys, in its parameters or its URL. Every path is read first as `httpx.URL` builds it: `.` and `..` removed on the path as written, a `..` above the root included, then percent-decoded once, as httpx's `.path` is. It is read again as a server may then read it: `;` parameters dropped, slashes merged, and `.` and `..` resolved again. A path is decoded only once, so a double-encoded one such as `/v1%252Foccurrence%252Fsearch` does not count. A literal backslash is not read as a slash. In stored text, meaning records and receipt blobs, an `api.gbif.org` URL (with a trailing dot, a port, an empty port, or the ideographic or fullwidth dots that IDNA reads as `.`) is looked for as written and again after each of up to four percent-decoding steps, so a URL encoded inside another is split before each decoding; JSON's escaped slashes are undone at every step. A URL in stored text ends at whitespace, a quote, `<`, `>` or a backslash. Every record's strings, keys included, get this text test, and JSON inside them is parsed and read the same way. A GBIF record's strings, keys included, also count as paths: the whole string as an absolute URL on any host, a path from the root, a relative path resolved from the root, or a `//` reference, read both as a host and path and as a path; and every `/` token inside it as a path from the root, so `GET /v1/occurrence/search?catalogNumber=1` counts. The parent count reads each request as written and as `httpx` builds it, parameters merged, so `httpx.QueryParams` count too. Failed on any GADM call (a `gbif_gadm` source or `gbif-gadm` adapter): PLAN 4.8 does not use GADM, not even as a measurement (coordinator ruling, 2026-09-25). GADM is not an occurrence request, so it stays outside D4. A call held by policy sent nothing and is skipped. Species match is known by its tool (`taxonomy_verifier`), adapter (`species-match`) or path (`/v2/species/match`), and in #134's evidence by a `usage/<key>` locator or the evidence id its tool call names. Reporting any other GBIF call for the coordinator is the lab's own choice, since PLAN 4.8's table has no row for it. Not checked while the runner's count is unavailable, while tool calls are recorded (#134; S4's checks to come), or while other GBIF calls are reported; not built when the run records none of these |
...
~/specimen-golive/runs/<subject>/<UTC timestamp>/
  ...
  verdict.md  a person's verdict: written by a person, never by the runner;
              the subject report carries it after the next run, or at
              once with --report-only
  state/  the app's blobs and local database for this run, and the
          emulator's logs (initdb's log names the OS account), all
          unredacted: never shared or attached anywhere
...
The two real runs of `subject_105526321` (`20260923T211535Z` and
`20260925T094409Z`) predate identity and home-path redaction, and nothing has
re-redacted them. Re-redact them before sharing any of their files.
...
bound: an unknown outcome (coordinator ruling, 2026-09-23, which names the
bound but no figure) and, as the lab's own extension after #86, a call that
returned and then failed. PLAN 4.3 runs the lab's USD 5 share under
production's mechanism, where a call reserves the sum of its requests' bounds,
each at least 20,000 micro-dollars. The lab reads an attempt's bound as the
largest of:

- the step's reservation in the run's profile, when it has one:
  `execution.stage_cost_reservations` for the step, or else
  `execution.request_cost_reservation_micros`, as `workflow.py` reserves;
- PLAN 4.3's floor for a call's two requests, USD 0.04 (`request_limit=2`);
- the call's token limit at the highest known price, USD 0.0192
  (`total_tokens_limit=16000`).

Both limits are in `UsageLimits` at `production.py:749`, `harness.py:92` and
`first_pass.py:283` (on main `569c336`). That reading is the lab's, confirmed
by the coordinator on 2026-09-25 at 15:50Z (coordinator.md:448) as at least as
strict as the mechanism the lab's share runs under.
...
`--max-run-usd`, since completing the upload starts processing and paid calls
may have run. SAM 3 on
this workstation costs nothing. Every run's cost and the lab's running total
against its USD 5.00 share go into `run.json` and its report (G9, G30), even
when a phase or the lane's teardown fails.
...
- the administrator's identity: any email address (an ADC error names the
  ...
  appear: `asset.uploader`, `audit[].actor`, `Transcript.actor`,
  ...
  inner space, `=`, `#`, a quote, a comma, a zero-width space, or a byte-order
  mark after the first line) is refused; whitespace around a value is trimmed. A `NAME:value` line still passes, since `:` belongs to
  ...
  path, and writes nothing when the variable is unset, or the file is
...
Matching ignores case, and an email is also found percent-encoded, once or
more (`%40`, `%2540`).
Values under 8 characters are matched as whole words. Every artifact that
decodes as UTF-8 text is redacted, whatever its suffix; images and other
binary files are written as they came. The runner never reads
`.env` or `.logfire/`.
...
- Two short windows can still leak the SQL Connect emulator: a Ctrl-C inside
  `subprocess.Popen` after it has started the process, or one between the
  lane's `__enter__` returning and the runner registering its exit.
