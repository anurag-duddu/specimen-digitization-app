...
7. **Report.** The files below, written even when a phase or the lane's
   ...
   and written before it stops. The subject report is rebuilt from every run of that
...
- the step's reservation in the run's profile
  (`execution.stage_cost_reservations`), when it has one;
...
- the administrator's identity: any email address (an ADC error names the
  ...
  in a repository. It holds one bare value per line, in UTF-8 (a byte-order
  mark is allowed); a line holding `=`, `#`, a quote or a space is refused, so
  a misformatted file cannot pass and redact nothing. The runner checks where the file is before reading it,
