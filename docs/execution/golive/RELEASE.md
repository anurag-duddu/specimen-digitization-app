...
- The job writes a `data-released/v1` receipt holding the phase and public
  resource names only, then attests and uploads it.
- The workflow's admission job has no credentials. It runs the gate and waits
  ...
  stays until a later clean-up removes it with its tests.
