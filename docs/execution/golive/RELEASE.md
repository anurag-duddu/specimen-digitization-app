...
- Order: SAM 3, then the worker job, then the API.
  ...
  - `specimen-api`: when a revision already serves, the new revision gets 0%
    ...
    three checks run on the service URL. A candidate that fails readiness loses
    its tag, and the previous revision keeps all traffic. Its invoker policy
    must be exactly `allUsers`, unconditionally.
