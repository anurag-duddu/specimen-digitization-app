...
| `scripts/qa/live/human_review.py` 52 and 124 | `HUMAN-NO-AUTOMATIC-CLEARANCE`'s clause that correction and save never enable automatic clearance or approve institutional semantics or risk (`RELEASE_ACCEPTANCE.md` 252). The rest of the subcriterion stays: no clearance with mandatory values unresolved, an operational failure never labelled Deferred (QUE-005), and review state and history surviving | G1 | S2, after T3d (the coordinator's assignment of 2026-09-24) |
| `scripts/qa/live/human_review.py` 194-200 | Every run with `institutional_policy_approved`, `semantics_confirmed` and `human_approved` false and a null disposition (194-197), the `policy.py` gates listed above, and stopped at the review block, `processing_blocked` with `pilot_evidence_review_required` (198-200). The replacement keeps their rule that an operational block cannot qualify | G1 | S2, after T3d (the coordinator's assignment of 2026-09-24) |
| `scripts/qa/live/human_review.py` 103 and 168 | The frozen manifest as the cohort's anchor: its digest on every human case (103) and its specimens (168). The checks it anchors stay, re-anchored to the ten, the acceptance cohort in order: no duplicate or outside-cohort record (175), every one of the ten (169, 290), the evidence fields every record carries (172-173), and the original-bytes and collection-scope checks (177-190, 235-237). Fewer than ten human records, where the harness clears a record, is G1's change, not G2's | G1, G2 | S2, after T3d (the coordinator's assignment of 2026-09-24) |
...
| `scripts/qa/live/human_review.py` 298-304, `scripts/qa/live/acceptance.py` 512-517 | COHORT-BUDGET: the `cohort-budget/v2` ledger's version (300-301) and approval digest (303), and the case pending while the ledger is missing or not live (`acceptance.py` 516-517, kept pending at 304). G9's USD 25 ceiling takes its place. G30's per-call reservations stand (PLAN 4.3; the coordinator's ruling on the mechanism): COST-BOUNDS still carries them, with unknown effects kept reserved (`acceptance.py` 400-403, reached through 515). DEPLOYMENT-PROVENANCE, which 304 also keeps pending, stays | G9, G11 | S2, after T3d (the coordinator's assignment of 2026-09-24) |
...
| `scripts/qa/live/human_review.py` 333-334, `scripts/qa/live/acceptance.py` 545-548 | The approved manifest's digest pinned on the command line, and the manifest read under it; re-anchored to the ten | G2 | S2, after T3d (the coordinator's assignment of 2026-09-24) |
| `scripts/qa/live/human_review.py` 105-108 and 123, `scripts/qa/live/acceptance.py` 449-452 and 476 | Coverage drawn from the manifest: each case's specimens inside it and without duplicates (105-108; `acceptance.py` 449-452), and full coverage of its ten (123 for the manual subcriteria; `acceptance.py` 476 for the full-cohort cases). They stay, re-anchored to the ten; fewer than ten records in a manual subcriterion, where the harness clears a record, is G1's change | G1, G2 | S2, after T3d (the coordinator's assignment of 2026-09-24) |
