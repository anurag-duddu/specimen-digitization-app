...
- Its requests carry place text only (coordinator rulings of 2026-09-24 and
  #174's final review, including the ruling on S8's option (c) for
  expansions). The agent keeps a place-lookup tool it can call mid-run (G40),
  ...
  for every request its tiers send. The rule covers every value a request
  takes from the record or a tier-1 result, in any parameter, and the
  filter's output is what leaves.
  - Sources, checked first, before any cut or expansion: exact substrings of
    ...
    any field); the names tier 1 returns; and, in "fill the rest", the
    reviewer's value in a place field. A value drawn from anything else is
    refused.
  ...
  - Expansion, after the cuts: a surviving notation token that the table
    ...
    never brings back a cut token. A full form with no such notation in a
    source is refused (coordinator ruling on S4's #183).
  - Fixed parts: SPARQL properties such as P625, P582 and P1365, paging,
    ...
    reviewed constants, not record values, with a test that they carry no
    label text; a record value enters SPARQL only as an escaped literal. The
    key is the Secret Manager credential, which the filter leaves untouched;
    the fixed-parts test uses a fake key, and no test or fixture records a key
    or the URL carrying it.
  ...
  - Tests show exactly what it guarantees: "H. Hoogstraal leg.", "3 Sept.
    ...
    quote in it reaching SPARQL escaped. No cut token leaves, an expansion
    carries only its full form, and a value not drawn from those sources is
    refused.
  - Its stated limit: text the filter cannot recognize, such as a name no
    ...
    and the month-position cut can trim a tier-1 name whose numeral stands
    beside a number, such as a region written with its code.
