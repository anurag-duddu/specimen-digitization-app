...
| A curator's confirmation of S8's curated place and itinerary entries (G36): the Insects collection manager, or a curator they name | the six McKinley slides and the Apo slide settling | S8 drafts a review sheet per entry with its sources; the coordinator routes it through the owner |
| Access to Getty TGN (G35), if its gateway needs an account or a token | tier 1 of the place tool | S8 states what access needs; the owner creates any account |
...
| Copernicus GLO-30 (D11) | nothing: tiles are read from the project's storage | the elevation sampled at open-source coordinates, with the tile's version | the notice its licence requires, as #94 gives |
