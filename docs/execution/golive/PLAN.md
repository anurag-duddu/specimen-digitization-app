...
G35's diagram, as the owner wrote it:
...
| S8's D4, the museum's published GBIF points, and D5, the checks' limits (#94) | the checks that compare a place with the label; until decided they record findings only and never change an outcome | put to the owner on 2026-09-24 and dismissed, so on hold until the owner takes them up; D1, D2, D3, D6, D7 and D9 are decided (G35 to G39), D11 and D13 are coordinator rulings, and D8, D10 and D12 wait for their phase, except Getty TGN (G35) |
...
arrives (G8), and the four elevation fields stay mandatory, filled like other fields with
authority and evidence (G37, revising G22). The profile carries the Insects date rules as versioned rules, which S3
...
section 4 specifies, not that every record cleared. Under G37 the harness now fills a missing elevation
or unit from settled fields with evidence, but the six McKinley slides go to
