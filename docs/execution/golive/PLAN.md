...
- Its requests carry place text only (coordinator rulings of 2026-09-24, from
  ...
  escaped or encoded.
  - Sources, checked first, before any cut or expansion: whole-token
    substrings, bounded by token edges, of the reading's place fields
  ...
  - Cuts, applied only to those values and by character span, so no
    character a cut covers can leave however a value is sliced: every token
...
| geoBoundaries, open release, and CONRED's departments via HDX (place tool, containment, G37) | nothing: files are read from the project's storage | the county or city whose unit holds the whole uncertainty circle, widened by the file's stated simplification error when a simplified file is used | per file, as each source's licence states: the Philippine units and Guatemala's municipios under CC BY 3.0 IGO (NAMRIA, PSA and OCHA Philippines; CONRED and OCHA FISS, via HDX), and Guatemala's departments from CONRED's CC BY 3.0 IGO release, never geoBoundaries' ODbL OpenStreetMap file (coordinator ruling, from S8's licence check of 2026-09-24) |
