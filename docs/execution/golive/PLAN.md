...
- Its requests carry place text only. The agent keeps a place-lookup tool it
  ...
  request it sends, its parameters included. A request draws only on exact
  substrings of the reading taken from its place fields (`country`,
  ...
  assigns to any field), on the names tier 1 returns, and, in "fill the rest",
  on the reviewer's value; a query drawn from anything else is refused. From
  what leaves, and only there, the filter cuts every token of every literal
  any reading assigns to a non-place field; every token of every clause,
  ...
  the profile's date notations list. Dates and elevations are compared
  locally and never sent. It applies to every request the place tool makes,
  tier 1, tier 2 and "fill the rest" alike. Tests show exactly what it
  guarantees, with "H. Hoogstraal leg." and "3 Sept. '46" among the cases: no
  such token leaves, and a query not drawn from those sources is refused. Text
  the filter cannot recognize, such as a name no reading assigns to any field
  and no marker accompanies, can still leave; that is its stated limit.
