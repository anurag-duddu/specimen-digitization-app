...
is the only screen with no app bar). The card spans roughly x=264–536 of the
...
(y=0–6% of viewport height) with a refresh icon and a "Sign out" icon at the
far right; a full-width yellow "SYNTHETIC ENVIRONMENT — fixture results are
...
A centered modal dialog over a dimmed background, roughly x=228–572 (43% of
viewport width) and y=13–487 (nearly the full viewport height, 91%). Title
