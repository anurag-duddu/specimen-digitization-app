...
### Composition
- `UiStickyBar` is new (`controls/navigation/sticky_bar.dart`, 13 section 3.5): a pinned sliver of one fixed extent for the row that scrolls up to the header and then sticks, drawn solid on `ground` and carrying the `header` marker; added by the integrator at the wave A merge so the record's segments and the queue's search row are the same bar. **Polish 3:** the bar is chrome only while it is stuck (13 section 3.5). In the flow of the page it paints nothing and lets the sky through, and it takes its `ground` fill once it is pinned, scrolled past the top of the viewport or held under a pinned header through the overlap, at the threshold and without fading (09 section 11), the way a collapsing header takes its chrome fill. Reason: the queue's search row, stuck at medium, drew a band of ground across the home sky while it was still a row of the page. The fill is one decoration whatever the state, so the row inside keeps its state at the threshold.
...
- `UiDecisionBar` takes `tertiary` actions the way `UiButtonRow` does, and the reason a move is absent (polish 3). The tertiary actions are drawn before the secondary in reading order and are the first to move into the bar's overflow menu, so a record whose approval has a prerequisite offers the save, the approval and the confirmation from one bar rather than choosing two; the menu keeps their labels, glyphs and reasons. `previousDisabledReason` and `nextDisabledReason` draw the edge control disabled with the reason on its hint and its tooltip where the move is null, so the end of a queue says why rather than falling silent, and a control with neither a move nor a reason is not drawn at all. The bar measures its buttons through `UiButton.intrinsicWidth`, which counts a glyph, where it used to measure the label alone: intake's single upload action overflowed a phone by five pixels at 200 percent text because the bar chose an arrangement that did not fit. At the last resort the primary and the count share the line three to one, and a bar with no count lets the primary take it, so a bar carrying only a primary ellipsises the way a bar carrying two does. The secondary drawn as a ghost keeps its glyphs, its loading state and its own semantics label.
- `UiStatusStrip.provenance` is new: the facts as slots (polish 3). A fact can be the product's glossary term, opening its definition on the line it is read on, or carry a tooltip; the strip sets them on one line as one paragraph of inline widgets, separated as before and ellipsised at the end, so a fact is never dropped ahead of the ones before it. Each is its own semantics node, because a paragraph merges an inline widget's semantics into its own unless the widget is a boundary, and the separators are drawn and not spoken. Each takes the strip's fact style unless it sets its own. Reason: the record's three facts are glossary terms, and a design system that took strings kept their definitions two taps away. `facts` keeps its strings for one version, drawn as a `Text` of each through the same paragraph, because the record's call site still passes them and an annotation would fail the application's analysis; a strip passes one of the two, and `facts` goes in the next minor version.
...
- `UiScaffold` draws every pane it owns solid while a modal shown from inside it is over it (13 section 2.2; 09 section 3.3; polish 3). The import sheet at compact drew its `glass.modal` pane over the navigation the frame still frosted beneath the scrim, two on a phone whose budget is one, and 13 section 2.2 exempts a sheet's scroll, not its pane. The frame owns a `UiModalScope`, publishes it to its page, and reads it: from the moment the modal's entrance has finished until its exit begins, the action bar, the pill, the body and the beside navigation draw the solid form of their surfaces, so the one save layer on the window is the modal's. A pane under a scrim is a save layer nobody sees.
- `UiScaffold`'s action bar has two forms (polish 3). Above a pill it floats as a tile with the pill's gap under it, as before, because the pill is the lowest chrome and a capsule floats over the page by design (10 section 4.4). Where nothing floats under it, inside a record where the pill is hidden or beside a rail or a sidebar, the bar is the lowest chrome and anchors to the window's bottom edge: the pane spans the body's width with its top corners turned at `shape.tile`, and the bottom system inset is padding inside the pane below the bar, so the page's content never shows in a band between the bar and the edge. Reason: the device captures at 4735cfa found about 48 dp of the record's evidence scrolling under the decision bar on a phone with gesture navigation and the readings' footer rows under it on a tablet with a home indicator. The `actionBar` marker now sits on the bar and its padding inside the pane, so the chrome budget still counts the 64 dp of 13 section 4.1 and never the device's inset; `UiScaffoldGeometry.bottomInset` is the whole pane, inset included, so the body clears it rather than the bar alone; and the keyboard lifts the anchored pane onto its own edge with no inset inside, since it covers the home indicator. Found on the way: the floating chrome's side padding added the side safe areas a second time, on top of the body area that already clears them, and is now the gutter alone.
...
- The Composition page pictures a record's bar with its trailing menu open (polish 3): a phone frame carrying an `Overlay` of its own, so the pane is drawn inside the frame where the golden captures it and where the popover measures the window it has to fit, with the menu mirrored onto the trigger's trailing edge where it used to open 250 dp past the window. The page's own pane count is four: the two record frames' action bars, the open menu, and the shell's page list where the sidebar is drawn.
...
- `UiModalScope` is new in `primitives/modal_routes.dart` (13 section 2.2; 09
  section 3.3; polish 3): where the package's modals say that one of them is
  open. `showUiSheet`, `showUiDialog` and `showUiModal` find the nearest scope
  above the context they were shown from, hold it from the moment the
  entrance has finished to the moment the exit begins, and give it back; a
  frame publishes one to its page with `UiModalScope.publish` and reads
  `isOpen`. Null outside a frame, so a modal shown from a component test
  changes nothing. Reason: a sheet is pushed on the root navigator over the
  whole frame, and nothing beneath a route can read the route above it; the
  caller's context is the one thing both sides hold. The frame safe deferral
  the scaffold's slots and exclusion used is now the one `FrameSafeNotifier`
  mixin the scope shares, unexported.
