...
- `UiStatusStrip.facts` is a list of widgets rather than strings (polish 3). A fact can be the product's glossary term, opening its definition on the line it is read on, or carry a tooltip; the strip sets the facts on one line as one paragraph of inline widgets, separated as before and ellipsised at the end, so a fact is never dropped ahead of the ones before it. Each fact is its own semantics node, because a paragraph merges an inline widget's semantics into its own unless the widget is a boundary, and the separators are drawn and not spoken. Each fact takes the strip's fact style unless it sets its own. Reason: the record's three facts are glossary terms, and a design system that took strings kept their definitions two taps away.
