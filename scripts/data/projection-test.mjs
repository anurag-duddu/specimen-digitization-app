...
ok(await raw(`mutation @transaction {
 ...
 s6: specimen_insert(data:{${sc},id:"${other}",revision:1,state:"completed",sensitive:false,createdBy:"worker"})
...
async function chain(specimenId, actorUid) {
  ...
  w.evidence = {...v, id: id(), runId: w.run.id, source: 'google-maps-geocoding', sourceVersion: 'v1', adapterVersion: 'a1', query: {address: 'Chicago, Ill.'}, outcome: 'success', locator: 'place/fixture-place', responseSha256: hex('e'), capturedAt: '2026-09-23T12:00:00Z', rawAssetId: w.envelope.id};
...
denied(await op('AppendReadingComparisonV1', {...work.comparison, id: randomUUID(), rightObservationId: work.third.id, editDistance: -1}));
denied(await op('AppendReadingComparisonV1', {...work.comparison, id: randomUUID(), rightObservationId: work.third.id, lengthBasis: 0}));
denied(await op('AppendReadingComparisonV1', {...work.comparison, id: randomUUID(), leftObservationId: work.right.id, rightObservationId: work.left.id}));
denied(await op('AppendReadingComparisonV1', {...work.comparison, id: randomUUID(), rightObservationId: work.firstPassCall.id}));
...
ok(await op('AppendValidationFindingV2', {...work.finding, id: randomUUID(), severity: 'warning', ruleId: 'taxonomy_source_disagreement', fieldKey: 'taxon', reasonCode: 'taxonomy_source_disagreement:taxon'}));
...
console.log('PASS closed vocabularies, consistent decisions, one pair order, image dimensions and the Google locator are enforced');
...
for (const [name, key] of writes) {
  ...
  if (name === 'AppendReadingComparisonV1') variables.rightObservationId = kept.third.id;
...
const crossings = [
  ...
  ['AppendReadingComparisonV1', {...work.comparison, id: rid(), rightObservationId: b.third.id}],
  ...
  ['AppendEvidenceItemV2', {...work.evidence, id: rid(), rawAssetId: kept.envelope.id}],
...
console.log('PASS parents of another run or specimen, sensitive ones included, are refused');
