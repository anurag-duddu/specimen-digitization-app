...
from test_evidence_pilot import pilot
...
def mapped_pilot(tmp_path, monkeypatch, *, limit=1000):
    ...
    specimen.run.profile.execution = ExecutionPolicy.model_validate(
        dict(
            specimen.run.profile.execution.model_dump(),
    ...
    repo.save(principal, specimen, specimen.version, "map-fixture", digest(cost_map()))
...
def test_exact_map_reserves_each_stage_before_effect_and_survives_restart(tmp_path, monkeypatch):
    ...
    for _ in range(10):
        ...
        if result.run.stage == "processing_blocked":
            break
    assert seen == [("segment", 17), ("handwriting-qwen", 76), ("handwriting-muse", 165)]
...
def test_known_retry_reserves_again_and_budget_stops_before_next_effect(tmp_path, monkeypatch):
    repo, principal, specimen, launch, workflow, _ = mapped_pilot(tmp_path, monkeypatch, limit=200)
    original = workflow.adapters.transcribe
    seen = []
    current = datetime.now(timezone.utc)
    workflow.clock = lambda: current
    workflow.random_value = lambda: 0

    def read(item, region, route):
        seen.append((route, repo.get(principal.scope, item.id).run.usage.reserved_cost_micros))
        if len(seen) == 1:
            from specimen_digitization.application.domain import LookupStatus
            raise AdapterFailure("fixture_rate_limit", LookupStatus.RATE_LIMITED)
        return original(item, region, route)

    workflow.adapters.transcribe = read
    for _ in range(12):
        result = workflow.step(principal, specimen.id)
        ...
        if result.run.stage == "processing_blocked":
            break
    assert seen == [("handwriting-qwen", 76), ("handwriting-qwen", 135)]
    assert result.run.blocker == "cost_budget_exhausted"
    assert result.run.usage.reserved_cost_micros == 135
    assert len(result.run.observations) == 1
    assert len(result.run.regions) == 1  # Budget never shrinks the region denominator.


def test_unknown_outcome_keeps_reservation_and_never_replays_after_restart(tmp_path, monkeypatch):
    repo, principal, specimen, launch, workflow, _ = mapped_pilot(tmp_path, monkeypatch)
    seen = []
    ...
    workflow.adapters.transcribe = unknown
...
def test_launch_and_run_map_must_match_exactly_before_any_effect(tmp_path, monkeypatch, change):
    repo, principal, specimen, launch, workflow, calls = mapped_pilot(tmp_path, monkeypatch)
    value = cost_map()
    if change == "missing":
        value = None
    elif change == "lower":
...
def test_revised_launch_map_cannot_reset_the_existing_cohort_ledger(tmp_path, monkeypatch):
    repo, principal, specimen, launch, workflow, _ = mapped_pilot(tmp_path, monkeypatch)
    workflow.admission.admit(repo.get(principal.scope, specimen.id))
    revised = cost_map()
    revised["cost_micros"]["segment"] = 1
    launch = PilotLaunch.model_validate(dict(launch.model_dump(), stage_cost_reservations=revised))
    specimen = repo.get(principal.scope, specimen.id)
    specimen.run.profile.execution = ExecutionPolicy.model_validate(
        dict(specimen.run.profile.execution.model_dump(), stage_cost_reservations=revised)
    )
...
def test_map_enabled_pilot_requires_versioned_complete_exact_stage_set(tmp_path, monkeypatch, change):
    ...
    with pytest.raises(ValidationError):
        PilotLaunch.model_validate(dict(launch.model_dump(), stage_cost_reservations=value))
