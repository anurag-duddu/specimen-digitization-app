...
| `worker_actor_uid` | The Firebase UID of the worker's own account, which is disabled or has no sign-in provider, and has no email (`docs/execution/golive/WORKER_MEMBERSHIP.md`). It needs that document's nonsensitive operator membership in the bootstrapped collection. | `runtime-activate` | The UID the owner stored as `specimen-worker-actor-uid` in Secret Manager. T3e's bootstrap run looks it up read-only (`accounts:lookup` with `{"localId":["UID"]}`) and refuses unless the account exists, is disabled or has no sign-in provider, and has no email. Owner-side evidence: on 2026-09-23 the coordinator looked the UID up read-only with the owner's credentials: found, disabled, 0 sign-in providers, no email, password or phone |
