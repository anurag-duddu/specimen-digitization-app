...
| `worker_actor_uid` | The Firebase UID of the worker's own account, which has no password and no sign-in provider, or is disabled, and is not a person's mailbox (`docs/execution/golive/WORKER_MEMBERSHIP.md`). It needs that document's nonsensitive operator membership in the bootstrapped collection. | `runtime-activate` | The UID the owner stored as `specimen-worker-actor-uid` in Secret Manager. The owner confirms the account with the `admin_uid` lookup, sending `{"localId":["UID"]}` in place of the email |
