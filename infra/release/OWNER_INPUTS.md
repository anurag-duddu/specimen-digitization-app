...
| `bootstrap_artifact_sha256` | The preparer's digest of the whole artifact. It also fills `bootstrap.sha256` and the candidate's `approvals.bootstrap_sha256`. 2026-09-24: this is not the approved digest of the go-live bootstrap run; see the note below. | `data-bootstrap`, `evidence-digests` | `jq -r .artifact_sha256 <prepared artifact>` |

2026-09-24, go-live program: superseded for the gate path by
[`docs/execution/golive/RELEASE.md`](../../docs/execution/golive/RELEASE.md)
section 4.5 (T3e). The protected data release now runs the bootstrap from its
gate record, with no plan and no envelope, so the steps above that copy the
artifact into `bootstrap.payload` and fill `bootstrap.sha256` with
`bootstrap_artifact_sha256` apply only to the envelope phases, until T3
removes them. For the bootstrap run the owner supplies three `data-production`
environment secrets instead, set for that run and deleted afterwards, and
opens the time-bounded `specimenDataOwnerBootstrap` window for it:

- `DATA_BOOTSTRAP_ARTIFACT_B64`: the base64 of the private artifact file's
  exact bytes, as `bootstrap_admin.py --hierarchy` wrote it, never a
  re-serialized copy. Line breaks in the base64 are ignored.
- `DATA_BOOTSTRAP_APPROVED_SHA256`: the digest that
  `uv run python scripts/data/hierarchy_approval.py summarize <private artifact>`
  writes to `hierarchy-approval.sha256` beside the artifact once the owner has
  read its summary and typed `APPROVE`: the SHA-256 of those exact bytes. It
  is not the artifact's internal `artifact_sha256` (`bootstrap_artifact_sha256`
  above); with that value the release refuses the artifact.
- `DATA_WORKER_ACTOR_UID`: the worker's UID, the value of `worker_actor_uid`
  below.
...
| `worker_actor_uid` | The Firebase UID of the worker's own account, which is disabled and has no email, password, phone or sign-in provider: the worker never signs in, it acts through its service account (`docs/execution/golive/WORKER_MEMBERSHIP.md`). It needs that document's nonsensitive operator membership in the bootstrapped collection. | `runtime-activate` | The UID the owner stored as `specimen-worker-actor-uid` in Secret Manager. Check: `curl -s -H "Authorization: Bearer $(token)" -H "Content-Type: application/json" -d '{"localId":["UID"]}' "https://identitytoolkit.googleapis.com/v1/projects/$PROJECT/accounts:lookup" \| jq -e '.users[1] == null and .users[0].disabled == true and .users[0].email == null and .users[0].phoneNumber == null and .users[0].passwordHash == null and .users[0].passwordUpdatedAt == null and (.users[0].providerUserInfo // []) == []'` exits 0. T3e's bootstrap run repeats the lookup read-only when it writes the membership and refuses unless the account exists, is disabled, and has no email, password, phone or sign-in provider. Owner-side evidence: on 2026-09-23 the coordinator looked the UID up read-only with the owner's credentials: found, disabled, 0 sign-in providers, no email, password or phone. 2026-09-24: T3e runs that lookup before every membership step, a verify's too (RELEASE.md 4.5 and the S2 plan), since the account may change after the write; the bootstrap run reads this UID from the `data-production` secret `DATA_WORKER_ACTOR_UID` |
